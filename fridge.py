...
class Fridge:
    ...
    def load_ingredients(self):
        """냉장고에 있는 재료 목록을 DB에서 로드하여 표시합니다."""
        # 기존에 있던 위젯들 삭제
        for widget in self.frame.winfo_children():
            widget.destroy()

        # 제목 레이블 추가
        title_label = tk.Label(self.frame, text="냉장고 재료 목록", font=("Helvetica", 16, "bold"), bg="#E3F2FD", fg="#0D47A1")
        ...
        # 냉장고 재료 목록을 DB에서 가져오기 (냉장고에 해당하는 재료만)
        cursor.execute("SELECT id, name FROM ingredients WHERE storage_type_id = (SELECT id FROM storage_types WHERE section_name = '냉장')")
    ...
    def remove_ingredient(self):
        ...
        messagebox.showinfo("냉장고", "선택한 재료가 냉장고에서 삭제되었습니다!")
        self.load_ingredients()  # 재료 삭제 후, 목록 갱신


# 예시로 Fridge 클래스 사용 (루트 윈도우 생성)
if __name__ == "__main__":
    root = tk.Tk()
    root.title("Fridge Example")
    ...
    fridge = Fridge(root, user, password)
