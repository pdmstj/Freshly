...
class Fridge:
    ...
    def load_ingredients(self):
        ...
        # 냉장고 재료 목록 DB에서 가져오기 (냉장고에 해당하는 재료만)
        cursor.execute("SELECT id, name FROM ingredients WHERE storage_type = '냉장'")
    ...
    def remove_ingredient(self):
        ...
        for ingredient_id, var in self.checkbuttons:
            if var.get() == 1:  # 체크박스가 선택되었을 경우
                # 냉장고에서 재료 삭제 쿼리
                cursor.execute(
                    "DELETE FROM ingredients WHERE id = %s AND storage_type = '냉장'",
                    (ingredient_id,),
                )
